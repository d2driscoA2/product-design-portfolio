...
const data: CaseStudyData = {
  ...
  headline: 'Designing for the Adversary: Making ML Fraud Scores Readable to Analysts Deciding in Seconds',
...
The risk model was not wrong. Analysts could not see its reasoning. The design problem was transparency.`,
    ...
    },
    {
      ...
      body: `I designed the dashboard to show the signals behind each risk score, not only the score. SIM swaps, breached data, porting history, location anomalies, carrier mismatches. All visible at a glance.

The rejected direction: a simplified mode toggle. Testing showed that switching modes broke the analyst's workflow. The final design put both layers on one screen. An allow, flag, or block recommendation with the three strongest signals on top, and the full parameter set collapsed below.
...
Round 3 established that risk scores needed to feel like risk. Early designs used a neutral palette. Analysts said the interface felt flat. They had to work to feel the urgency of a high-risk score. The final design used saturated red for high-risk, amber for borderline, neutral for low. Two analysts flagged that color alone was unreliable across screens with different calibrations. I added a segmented bar as a secondary indicator, independent of color.
...
Transparency is a trust mechanism. Analysts do not trust systems they cannot interrogate. A risk score without a visible reason is a black box with a number on it.
